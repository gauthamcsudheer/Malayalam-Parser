...
\end{itemize}
	...
	\paragraph\ 
		...
		\section{Overview}
		\paragraph\
		Our project provides three essential datasets for Malayalam text analysis: POS-tagged, named entity labeled, and sentiment labeled.We also include POS tag information and examples from the datasets to make it easier for users to understand the tags.The implementation of the parser involved the use of the translation method.Additionally, we tried implementing with rule-based approaches and a bidirectional LSTM model, but did not integrate them into the main project due to accuracy concerns.Despite this, the project lays a solid foundation for processing and analyzing Malayalam text and has the potential to advance NLP applications for the language.
		...
		\paragraph\
			...
			The project has developed a Malayalam Parser for Dataset Creation.To improve accuracy, we employed rule-based and a bidirectional LSTM model.Although the LSTM model achieved 72.11\% accuracy  and a 21.77\% F1 score, it did not meet our required accuracy level.Due to time constraints, further enhancement of the model remains a future consideration.
